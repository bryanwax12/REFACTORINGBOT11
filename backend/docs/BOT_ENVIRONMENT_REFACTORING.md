...
```bash
curl https://shipping-rates-4.preview.emergentagent.com/api/bot-config/status
```
...
```bash
curl https://shipping-rates-4.preview.emergentagent.com/api/bot-config/full \
...
```bash
curl https://shipping-rates-4.preview.emergentagent.com/api/bot-config/recommendations
```
...
```bash
curl https://shipping-rates-4.preview.emergentagent.com/api/bot-config/webhook-info \
  -H "X-API-Key: YOUR_ADMIN_KEY"
```
...
```bash
curl -X POST https://shipping-rates-4.preview.emergentagent.com/api/bot-config/switch-environment \
  -H "X-API-Key: YOUR_ADMIN_KEY" \
...
```bash
curl -X POST https://shipping-rates-4.preview.emergentagent.com/api/bot-config/switch-mode \
  -H "X-API-Key: YOUR_ADMIN_KEY" \
  -H "Content-Type: application/json" \
  -d '{"mode": "webhook"}'
```
