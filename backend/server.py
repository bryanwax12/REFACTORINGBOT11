...
# ✅ STATE VALIDATION: All valid states for order flow
# Used by return_to_order and other handlers to validate state transitions
ORDER_FLOW_STATES = [
    ...
    CALCULATING_RATES, CONFIRM_DATA, EDIT_MENU, SELECT_CARRIER, PAYMENT_METHOD
]
...
async def create_and_send_label(order_id, telegram_id, message):
    try:
        ...
        from repositories import get_repositories
        repos = get_repositories()
        update_result = await repos.orders.update_by_order_id(order_id, {
            "shipping_status": "label_created",
            "tracking_number": tracking_number,
            "label_id": label_id,
            "shipment_id": shipment_id
        })
        logger.info(f"📝 Order updated with tracking info: {update_result}")
