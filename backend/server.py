...
FROM_NAME, FROM_ADDRESS, FROM_ADDRESS2, FROM_CITY, FROM_STATE, FROM_ZIP, FROM_PHONE, TO_NAME, TO_ADDRESS, TO_ADDRESS2, TO_CITY, TO_STATE, TO_ZIP, TO_PHONE, PARCEL_WEIGHT, CONFIRM_DATA, EDIT_MENU, SELECT_CARRIER, PAYMENT_METHOD = range(19)
...
async def create_nowpayments_payment(
    ...
    """Create a payment through NOWPayments API"""
    import requests
    ...
    # Get IPN callback URL (will be configured later)
    ipn_callback_url = os.environ.get("IPN_CALLBACK_URL", "https://yourdomain.com/api/webhooks/nowpayments")
...
async def get_nowpayments_status(payment_id: str) -> dict:
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(f"NOWPayments status check error: {response.text}")
        ...
        try:
            ...
            order_conv_handler = ConversationHandler(
                ...
                states={
                    ...
                    PAYMENT_METHOD: [CallbackQueryHandler(process_payment, pattern='^(pay_from_balance|pay_with_crypto|crypto_|top_up_balance|topup_|cancel_order)')]
                },
                fallbacks=[
