...
async def create_oxapay_invoice(amount: float, order_id: str, description: str = "Shipping Label Payment"):
    ...
    try:
        ...
        payload = {
            ...
            "callback_url": f"{os.environ.get('WEBHOOK_BASE_URL', 'https://telebot-refactor.preview.emergentagent.com')}/api/oxapay/webhook",
...
async def order_from_city(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Save to session
    user_id = update.effective_user.id
    await save_to_session(user_id, "FROM_STATE", {'from_city': city}, context)
...
async def order_from_zip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Save to session
    user_id = update.effective_user.id
    await save_to_session(user_id, "FROM_PHONE", {'from_zip': zip_code}, context)
...
async def order_to_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Save to session
    user_id = update.effective_user.id
    await save_to_session(user_id, "TO_ADDRESS", {'to_name': name}, context)
...
async def order_to_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Save to session
    user_id = update.effective_user.id
    await save_to_session(user_id, "TO_ZIP", {'to_state': state}, context)
