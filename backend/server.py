...
async def create_oxapay_invoice(amount: float, order_id: str, description: str = "Shipping Label Payment"):
    ...
    try:
        ...
        payload = {
            ...
            "callback_url": f"{os.environ.get('WEBHOOK_BASE_URL', 'https://swiftbot.preview.emergentagent.com')}/api/oxapay/webhook",
...
async def debug_logs(
    ...
    """
    Debug endpoint to view production logs in real-time
    ...
    # Get logs
    logs = memory_handler.get_logs(level=level, limit=limit)
    
    # Get application status
    app_status = {
        "application_initialized": application is not None,
        "bot_instance_initialized": bot_instance is not None,
        "webhook_mode": os.environ.get('WEBHOOK_URL') or os.environ.get('WEBHOOK_BASE_URL') is not None,
        "telegram_token_set": bool(TELEGRAM_BOT_TOKEN),
    }
    
    # Get active conversations (if application initialized)
    active_conversations = {}
    if application and hasattr(application, 'persistence'):
        try:
            # Try to get conversation data
            active_conversations = {"info": "ConversationHandler state tracking available"}
        except Exception as e:
            active_conversations = {"error": str(e)}
    
    # Get some stats from MongoDB
    try:
        users_count = await db.users.count_documents({})
        orders_count = await db.orders.count_documents({})
        recent_orders = await db.orders.count_documents({
            "created_at": {"$gte": datetime.now(timezone.utc).timestamp() - 3600}
        })
    except Exception as e:
        users_count = orders_count = recent_orders = f"Error: {e}"
...
async def debug_status(admin_key: str = Header(None, alias="X-Admin-Key")):
    """
    Quick status check without logs
    ...
    return {
        ...
        "webhook_base_url": os.environ.get('WEBHOOK_BASE_URL', 'Not set'),
        "telegram_token": TELEGRAM_BOT_TOKEN[:20] + "..." if TELEGRAM_BOT_TOKEN else "Not set",
        "recent_logs_count": len(memory_handler.logs),
        "last_log_time": memory_handler.logs[-1]['timestamp'] if memory_handler.logs else "No logs yet"
    }
