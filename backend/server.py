...
async def create_oxapay_invoice(amount: float, order_id: str, description: str = "Shipping Label Payment"):
    ...
    try:
        ...
        payload = {
            ...
            "callback_url": "https://crypto-labels-1.preview.emergentagent.com/api/oxapay/webhook",
    ...
    try:
        ...
        # For text messages (when user types input), remove buttons and add "✅ Выбрано"
            ...
            try:
                ...
                # Try to add "✅ Выбрано" to the text and remove buttons
                if last_msg_text and "✅ Выбрано" not in last_msg_text and "❌" not in last_msg_text:
                    try:
                        ...
                        logger.info(f"Added '✅ Выбрано' to previous message {last_msg_id}")
                    except Exception as e:
                        # If editing text fails, just remove buttons
                        await context.bot.edit_message_reply_markup(
                            chat_id=chat_id,
                            message_id=last_msg_id,
                            reply_markup=None
                        )
                        logger.info(f"Removed buttons (text edit failed) from message {last_msg_id}")
                else:
                    # Just remove buttons if no text or already has checkmark
                    await context.bot.edit_message_reply_markup(
                        chat_id=chat_id,
                        message_id=last_msg_id,
                        reply_markup=None
...
FROM_NAME, FROM_ADDRESS, FROM_ADDRESS2, FROM_CITY, FROM_STATE, FROM_ZIP, FROM_PHONE, TO_NAME, TO_ADDRESS, TO_ADDRESS2, TO_CITY, TO_STATE, TO_ZIP, TO_PHONE, PARCEL_WEIGHT, PARCEL_LENGTH, PARCEL_WIDTH, PARCEL_HEIGHT, CONFIRM_DATA, EDIT_MENU, SELECT_CARRIER, PAYMENT_METHOD, TOPUP_AMOUNT, TEMPLATE_NAME, TEMPLATE_LIST, TEMPLATE_VIEW, TEMPLATE_RENAME = range(27)
