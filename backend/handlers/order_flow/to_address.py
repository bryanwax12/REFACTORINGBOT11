...
async def order_to_city(update: Update, context: ContextTypes.DEFAULT_TYPE, session_service):
    ...
    if not is_valid:
        logger.warning(f"❌ VALIDATION ERROR [TO_CITY]: User {update.effective_user.id} entered '{city}' - Error: {error_msg}")
...
async def order_to_zip(update: Update, context: ContextTypes.DEFAULT_TYPE, session_service):
    ...
    if not is_valid:
        logger.warning(f"❌ VALIDATION ERROR [TO_ZIP]: User {update.effective_user.id} - Error: {error_msg}")
        try:
            error_sent = await safe_telegram_call(update.message.reply_text(error_msg), timeout=5)
            if not error_sent:
                error_sent = await update.message.reply_text(error_msg)
...
async def order_to_phone(update: Update, context: ContextTypes.DEFAULT_TYPE, session_service):
    ...
    if not is_valid:
        logger.warning(f"❌ VALIDATION ERROR [TO_PHONE]: User {update.effective_user.id} - Error: {error_msg}")
        try:
            error_sent = await safe_telegram_call(update.message.reply_text(error_msg), timeout=5)
            if not error_sent:
                error_sent = await update.message.reply_text(error_msg)
