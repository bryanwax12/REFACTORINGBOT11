...
async def refund_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = (
        ...
        "Если у вас не получилось отправить лейбл,\n"
        "свяжитесь с нашим агентом для помощи.\n\n"
        "📝 *Отправьте номер(а) лейбла для возврата:*\n"
        "_(можете указать несколько через запятую)_"
    )
    
    keyboard = [
        [InlineKeyboardButton("💬 Связаться с агентом", url="https://t.me/your_support_username")],
