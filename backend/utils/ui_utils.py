...
class TemplateEditMessages:
    ...
    FROM_NAME = step_message.__func__(1, 7, "👤 Имя отправителя\nНапример: John Smith")
    FROM_ADDRESS = step_message.__func__(2, 7, "🏠 Адрес отправителя\nНапример: 215 Clayton St.")
    FROM_ADDRESS2 = step_message.__func__(3, 7, "🏢 Адрес 2 (опционально)\nНапример: Apt 4B или Suite 200\nИли нажмите \"Пропустить\" ")
    FROM_CITY = step_message.__func__(4, 7, "🏙 Город отправителя\nНапример: San Francisco")
    FROM_STATE = step_message.__func__(5, 7, "📍 Штат отправителя (2 буквы)\nНапример: CA, NY, TX, FL")
    FROM_ZIP = step_message.__func__(6, 7, "📮 ZIP код отправителя\nНапример: 94102")
    FROM_PHONE = step_message.__func__(7, 7, "📞 Телефон отправителя (опционально)\nНапример: +11234567890 или 1234567890\nИли нажмите \"Пропустить\" ")
    
    # TO address edit steps (1-7 of 7)
    TO_NAME = step_message.__func__(1, 7, "👤 Имя получателя\nНапример: Jane Doe")
    TO_ADDRESS = step_message.__func__(2, 7, "🏠 Адрес получателя\nНапример: 123 Main St.")
    TO_ADDRESS2 = step_message.__func__(3, 7, "🏢 Адрес 2 получателя (опционально)\nНапример: Apt 4B\nИли нажмите \"Пропустить\" ")
    TO_CITY = step_message.__func__(4, 7, "🏙 Город получателя\nНапример: Los Angeles")
    TO_STATE = step_message.__func__(5, 7, "📍 Штат получателя (2 буквы)\nНапример: CA, NY, TX")
    TO_ZIP = step_message.__func__(6, 7, "📮 ZIP код получателя\nНапример: 90001")
    TO_PHONE = step_message.__func__(7, 7, "📞 Телефон получателя (опционально)\nНапример: +11234567890\nИли нажмите \"Пропустить\" ")
