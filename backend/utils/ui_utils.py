...
class OrderStepMessages:
    ...
    # FROM address steps
    FROM_NAME = step_message.__func__(1, 17, "👤 Имя отправителя\nНапример: John Smith")
    FROM_ADDRESS = step_message.__func__(2, 17, "🏠 Адрес отправителя\nНапример: 215 Clayton St.")
    FROM_ADDRESS2 = step_message.__func__(3, 17, "🏢 Адрес 2 (опционально)\nНапример: Apt 4B или Suite 200\nИли нажмите \"Пропустить\" ")
    FROM_CITY = step_message.__func__(4, 17, "🏙 Город отправителя\nНапример: San Francisco")
    FROM_STATE = step_message.__func__(5, 17, "📍 Штат отправителя (2 буквы)\nНапример: CA, NY, TX, FL")
    FROM_ZIP = step_message.__func__(6, 17, "📮 ZIP код отправителя\nНапример: 94102")
    FROM_PHONE = step_message.__func__(7, 17, "📞 Телефон отправителя (опционально)\nНапример: +11234567890 или 1234567890\nИли нажмите \"Пропустить\" ")
    
    # TO address steps
    TO_NAME = step_message.__func__(8, 17, "👤 Имя получателя\nНапример: Jane Doe")
    TO_ADDRESS = step_message.__func__(9, 17, "🏠 Адрес получателя\nНапример: 123 Main St.")
    TO_ADDRESS2 = step_message.__func__(10, 17, "🏢 Адрес 2 получателя (опционально)\nНапример: Apt 4B\nИли нажмите \"Пропустить\" ")
    TO_CITY = step_message.__func__(11, 17, "🏙 Город получателя\nНапример: Los Angeles")
    TO_STATE = step_message.__func__(12, 17, "📍 Штат получателя (2 буквы)\nНапример: CA, NY, TX")
    TO_ZIP = step_message.__func__(13, 17, "📮 ZIP код получателя\nНапример: 90001")
    TO_PHONE = step_message.__func__(14, 17, "📞 Телефон получателя (опционально)\nНапример: +11234567890\nИли нажмите \"Пропустить\" ")
    
    # Parcel steps
    PARCEL_WEIGHT = step_message.__func__(15, 17, "📦 Вес посылки (в фунтах)\nНапример: 5 или 5.5")
    PARCEL_LENGTH = step_message.__func__(16, 17, "📏 Длина посылки (в дюймах)\nНапример: 10 или 10.5")
    PARCEL_WIDTH = step_message.__func__(17, 17, "📐 Ширина посылки (в дюймах)\nНапример: 8 или 8.5")
    PARCEL_HEIGHT = step_message.__func__(18, 17, "📦 Высота посылки (в дюймах)\nНапример: 6 или 6.5")
