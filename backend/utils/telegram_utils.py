...
def get_random_emojis():
    """
    Get a pair of random fun emojis for thank you messages
    Получает пару случайных веселых эмоджи для благодарственных сообщений
    
    Returns:
        tuple: (left_emoji, right_emoji)
    """
    import random
    # Fun and cheerful emoji pairs
    emoji_pairs = [
        ("🎉", "🎊"),
        ("✨", "💫"),
        ("🌟", "⭐"),
    ...
    return random.choice(emoji_pairs)
...
async def generate_thank_you_message():
    ...
    Returns:
        str: Thank you message in Russian with fun emojis
    """
    try:
        import os
        ...
        chat = LlmChat(
            ...
            system_message="""Ты креативный помощник, создающий уникальные и теплые благодарности клиентам нашего сервиса на русском языке. 
...
Используй разные подходы: иногда официальный тон, иногда более дружеский, варьируй структуру предложений."""
        ...
        import random
        prompts = [
            "Создай краткое благодарственное сообщение клиенту за использование нашего сервиса (2-3 предложения). Используй тёплый, дружелюбный тон.",
            "Напиши короткое сообщение с благодарностью клиенту за выбор нашего сервиса. Сделай его искренним и приятным (2-3 предложения).",
            "Сформулируй благодарность клиенту за доверие к нашему сервису. Пусть сообщение будет тёплым, но лаконичным (2-3 предложения).",
            "Создай уникальное сообщение благодарности для клиента нашего сервиса. Тон - дружелюбный и позитивный (2-3 предложения).",
            "Напиши короткую благодарность клиенту за пользование нашим сервисом. Сделай текст живым и искренним (2-3 предложения)."
        ]
        ...
        if response and len(response.strip()) > 10:
            ...
            # Add random emojis around the message
            left_emoji, right_emoji = get_random_emojis()
            return f"{left_emoji} {response.strip()} {right_emoji}"
        else:
        ...
        # Use varied fallback messages with emojis
        import random
        fallback_messages = [
            "Спасибо за использование нашего сервиса! Желаем успешной отправки.",
        ...
        # Add random emojis
        left_emoji, right_emoji = get_random_emojis()
        return f"{left_emoji} {message} {right_emoji}"
