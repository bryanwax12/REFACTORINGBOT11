...
async def legacy_set_api_mode(req: Request, request: dict, api_key: str = Depends(verify_api_key)):
    ...
    from server import db, clear_settings_cache, ADMIN_TELEGRAM_ID, api_config_manager, bot_instance
    from handlers.common_handlers import safe_telegram_call
    import server
    import os
    
    logger.info(f"⚙️ [API_MODE] Changing API mode")
    logger.info(f"📋 [API_MODE] bot_instance available: {bot_instance is not None}")
...
async def legacy_enable_maintenance(request: Request, api_key: str = Depends(verify_api_key)):
    ...
    # CRITICAL: Use bot_settings collection (same as maintenance check)
    logger.info("🔧 legacy_enable_maintenance called")
    ...
    logger.info(f"✅ Maintenance enabled in bot_settings (matched: {result.matched_count}, modified: {result.modified_count})")
    clear_settings_cache()
